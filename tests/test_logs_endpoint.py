...
SENSITIVE_LOG_TEXTS: tuple[str, ...] = (
    ...
    "authToken=firebase-token-123",
    "password=hunter2",
    "apiKey=AIzaSyD-example-provider-secret",
    "sk-proj-abcdefghijklmnopqrstuvwxyz123456",
    "prompt: user meal description with private text",
    "message=chat message with private text",
    "text=meal description private text",
    'request body: {"meal":"oats"}',
    'rawBody={"meal":"oats"}',
    'response body: {"error":"secret"}',
    ...
    "meals%2Fuser-1%2Fimage.jpg",
    "avatars/user-1/avatar.abc123",
    "feedback/user-1/feedback-1/feedback.jpg",
    "feedback%2Fuser-1%2Ffeedback-1%2Ffeedback.jpg",
)
...
def _assert_mocked_logger_did_not_receive(raw_text: str, logged_payload: object) -> None:
    assert raw_text not in repr(logged_payload)
    assert "[REDACTED_" in repr(logged_payload)
...
def test_logs_error_endpoint_redacts_sensitive_message_stack_and_context_text(
    mocker: MockerFixture,
) -> None:
    reset_rate_limit_state()
    log_error = mocker.patch("app.api.routes.logs.error_logger.log_error")
    ...
    for raw_text in SENSITIVE_LOG_TEXTS:
        response = client.post(
            "/api/v1/logs/error",
            json={
                "source": "mobile",
                "message": f"client log {raw_text}",
                "stack": f"Error stack\n{raw_text}",
                "context": {"reason": f"context {raw_text}"},
            },
        )

        assert response.status_code == 201
        _assert_mocked_logger_did_not_receive(raw_text, log_error.call_args)

    assert log_error.call_count == len(SENSITIVE_LOG_TEXTS)


def test_logs_error_endpoint_rejects_raw_provider_context_string_values(
    mocker: MockerFixture,
) -> None:
    reset_rate_limit_state()
    log_error = mocker.patch("app.api.routes.logs.error_logger.log_error")
    ...
    response = client.post(
        ...
        json={
            "source": "mobile.ai",
            "message": "provider boundary regression",
            "context": {"reason": "secret-provider-prompt"},
        },
    )
