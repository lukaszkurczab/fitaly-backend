"""Storage rules emulator evidence for canonical avatar path isolation."""
...
AVATAR_BYTES = b"avatar-rules-emulator-image"
...
def test_avatar_storage_rules_enforce_canonical_owner_only_hash_path() -> None:
    run_id = uuid4().hex
    owner_email = f"ch-05-004c-avatar-owner-{run_id}@example.test"
    other_email = f"ch-05-004c-avatar-other-{run_id}@example.test"
    admin_email = f"ch-05-004c-avatar-admin-{run_id}@example.test"
    owner_token = ""
    other_token = ""
    admin_initial_token = ""
    admin_token = ""
    ...
    try:
        ...
        canonical_filename = f"avatar.{'a' * 64}"
        canonical_path = f"avatars/{owner_uid}/{canonical_filename}"
        other_user_path = f"avatars/{owner_uid}/{canonical_filename}"
        noncanonical_paths = [
            f"avatars/{owner_uid}/avatar",
            f"avatars/{owner_uid}/avatar.jpg",
            f"avatars/{owner_uid}/avatar.jpeg",
            f"avatars/{owner_uid}/avatar.abc123",
            f"avatars/{owner_uid}/avatar.{'A' * 64}",
            f"avatars/{owner_uid}/avatar-{'a' * 64}",
            f"avatars/{owner_uid}/profile.{'a' * 64}",
            f"avatars/{owner_uid}/nested/avatar.{'a' * 64}",
            f"avatars/{owner_uid}/avatar.{'a' * 63}",
            f"avatars/{owner_uid}/avatar.{'a' * 65}",
        ]
        ...
        assert owner_read_body == AVATAR_BYTES
        ...
        other_read_status, _ = _read_storage_object(other_user_path, id_token=other_token)
        ...
        admin_read_status, _ = _read_storage_object(canonical_path, id_token=admin_token)
        assert admin_read_status == 403

        for path in noncanonical_paths:
            read_status, _ = _read_storage_object(path, id_token=owner_token)
            assert read_status == 403
            assert (
                _upload_storage_object(
                    path,
                    id_token=owner_token,
                    body=b"noncanonical-avatar",
                )
                == 403
            )
    finally:
